...
const FoodAdd = () => {
  ...
  return (
    ...
    >
      {
        isLoading || add_food?.isLoading ?<Loader/>: <CForm onSubmit={formik?.handleSubmit}>
        <CCard className="mb-5 p-2">
          <CCardBody className=" d-flex justify-content-center align-items-center">
            {
              data?.data?.data?.length && data?.data?.data?.map((each)=>{
                return   <CFormCheck
                inline
                type="radio"
                id="inlineCheckbox1"
          ...
          {formik?.touched?.food_category && (
            <p className="text-danger text-center">{formik?.errors?.food_category} </p>
          )}
        </CCard>
        <CCard className="p-5">
          <CCardBody>
